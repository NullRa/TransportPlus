...
class UbikeStationAPI {

    func fetchStationList(stationType: CityType) throws -> [UbikeStation] {
        ...
        let dataList = try decoder.decode([UbikeStationJson].self, from: data)
        for station in dataList {
            let moc = CoreDataHelper.shared.managedObjectContext()
            let currentStation = UbikeStation(context: moc)
            currentStation.cityName = station.AuthorityID
    ...
    func fetchStationStatus(stationType: CityType, stationID: String) throws -> UbikeStateJson {
